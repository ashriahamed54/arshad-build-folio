            ...
            <div className="space-y-6 animate-slide-in">
              <Card className="p-8 bg-gradient-to-br from-primary to-primary/90 text-primary-foreground">
                ...
                <div className="space-y-6">
                  {contactInfo.map((item, index) => <div key={index} className="flex items-start gap-4">
                      <div className="p-3 rounded-lg bg-amber-500">
                        <item.icon className="h-5 w-5 text-accent-foreground" />
                      </div>
                      <div>
                        <p className="text-sm text-primary-foreground/70 mb-1">{item.label}</p>
                        {item.href ? <a href={item.href} target={item.label === "LinkedIn" ? "_blank" : undefined} rel={item.label === "LinkedIn" ? "noopener noreferrer" : undefined} className="font-semibold hover:text-accent transition-colors">
                            {item.value}
                          </a> : <p className="font-semibold">{item.value}</p>}
                      </div>
                    </div>)}
                </div>
              </Card>
            ...
            <div className="animate-scale-in">
              <Card className="p-8 h-full flex flex-col justify-center bg-muted/30">
                ...
                <div className="space-y-4">
                  <Button size="lg" className="w-full bg-accent hover:bg-accent/90 text-accent-foreground font-semibold" onClick={() => window.location.href = 'mailto:arshadmmashood@gmail.com'}>
                    <Mail className="mr-2 h-5 w-5" />
                    Send Me an Email
                  </Button>
                  <Button size="lg" variant="outline" className="w-full border-2 hover:bg-accent hover:text-accent-foreground hover:border-accent transition-all" onClick={() => window.location.href = 'tel:+94775127647'}>
